...
void CWb::Execute(udword_t* _nsteps)
{
  ...
  for(i = 0; i < WB_WIDTH; i++) 
  {
    ...
    if( robit->IsBadInst() ) 
    {
      sprintf(errtile, "(L2001-@%08lX, %08lX)", robit->Addr, robit->Inst);
      errmsg(__IDEC_H, ERR_NOTIMPLEMENTED, errtile);
    }

    if( robit->IsTraceRedundant() ) 
    {     
      // atualiza os registradores reais a partir do trace
#if(DEBUG == 0)
    ...
    {     
      // atualiza os registradores reais a partir de instrucoes simples
#if(DEBUG == 0)
