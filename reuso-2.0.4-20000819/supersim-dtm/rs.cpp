...
int CRsItem::IsReady()
{
  ...
  return ( !IsBusy() && 
	   ( !TraceData.IsLdStInst() || ( (TraceData.Rd_hi).IsReady() && (TraceData.Rd_lo).IsReady() ) ) );
}
...
void CRsItem::SetItem(CDisqItem _di)
{
  *((CDisqItem*) this) = _di;
  ...
  if( !IsTraceRedundant() ) {
    ...
    (TraceData.Rd_hi).Set(_di.Rd_hi);
    
    (TraceData.Rd_lo).Set(_di.Rd_lo);
  }
...
void CRs::FreeSeqNum(uword_t _seq)
{
  ...
  for(uword_t i = 0; i < GetQueueSize(); i++) 
  {
    ...
    {
      (p->TraceData).Rd_lo.DecDep(_seq);

      (p->TraceData).Rd_hi.DecDep(_seq);
    }
  }
}
		  ...
		  uword_t _val_y )
{
  ...
  for(uword_t i = 0; i < GetQueueSize(); i++) 
  {
	...
	if( ((p->Rs1).Update(_seq, p->GetSourceWin(), _cwp, _rd_hi, _val_hi) != TRUE) && (_rd_hi != _rd_lo) )
        {
	  (p->Rs1).Update(_seq, p->GetSourceWin(), _cwp, _rd_lo, _val_lo);
        }
      
	if( ((p->Rs2).Update(_seq, p->GetSourceWin(), _cwp, _rd_hi, _val_hi) != TRUE) && (_rd_hi != _rd_lo) )
        {
	  (p->Rs2).Update(_seq, p->GetSourceWin(), _cwp, _rd_lo, _val_lo);
	}

	if( ((p->Rd_hi).Update(_seq, p->GetSourceWin(), _cwp, _rd_hi, _val_hi) != TRUE) && (_rd_hi != _rd_lo) )
        {
	  (p->Rd_hi).Update(_seq, p->GetSourceWin(), _cwp, _rd_lo, _val_lo);
        }
      
	if( ((p->Rd_lo).Update(_seq, p->GetSourceWin(), _cwp, _rd_hi, _val_hi) != TRUE) && (_rd_hi != _rd_lo) )
        {
      ...
      {
	if( ((p->TraceData).Rd_hi.Update(_seq, p->GetSourceWin(), _cwp, _rd_hi, _val_hi) != TRUE) && (_rd_hi != _rd_lo) )
        {
	  (p->TraceData).Rd_hi.Update(_seq, p->GetSourceWin(), _cwp, _rd_lo, _val_lo);
        }

	if( ((p->TraceData).Rd_lo.Update(_seq, p->GetSourceWin(), _cwp, _rd_hi, _val_hi) != TRUE) && (_rd_hi != _rd_lo) )
        {
	  (p->TraceData).Rd_lo.Update(_seq, p->GetSourceWin(), _cwp, _rd_lo, _val_lo);
        }
      }
    }
  }
...
void CRs::UpdatePsr( uword_t _seq, uword_t _val_psr )
{
  CRsItem* p;
  for(uword_t i = 0; i < GetQueueSize(); i++) 
  {
...
void CRs::UpdateY( uword_t _seq, uword_t _val_y )
{
  CRsItem* p;
  for(uword_t i = 0; i < GetQueueSize(); i++) 
  {
		      ...
		      uword_t _rval )
{
  ...
  for(uword_t i = 0; i < GetQueueSize(); i++) 
  {
	...
	(p->Rd_lo).Update(_seq, p->GetSourceWin(), _cwp, _raddr, _rval);
      ...
      {
	(p->TraceData).Rd_hi.Update(_seq, p->GetSourceWin(), _cwp, _raddr, _rval);
	(p->TraceData).Rd_lo.Update(_seq, p->GetSourceWin(), _cwp, _raddr, _rval);
      }
    }
  }
}
...
void CRs::Show()
{
  ...
  for(uword_t i = 0; i < GetQueueSize(); i++) 
  {
	...
	if( (p->Rd_hi).IsReqAsSource() ) 
      ...
      {
	if( (p->TraceData).Rd_lo.IsReqAsSource() ) 
        {
	  if( (p->TraceData).Rd_lo.IsReady() )
	    ...
	    printf( "(rs%lu<-%lu) ", 
		    (p->TraceData).Rd_lo.GetRegAddr(),
		    (p->TraceData).Rd_lo.GetDep() );
          }
	}

	if( (p->TraceData).Rd_hi.IsReqAsSource() ) 
        {
