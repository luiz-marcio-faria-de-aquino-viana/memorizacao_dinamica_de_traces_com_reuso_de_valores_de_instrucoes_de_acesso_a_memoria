...
bool_t CDtm::IsValidDtmInst(CRobItem* _robit)
{
  switch((_robit->fi).op) 
  {
    ...
    {
      switch((_robit->fi).op3)  /* load integer instructions from alternate space */ 
      {
        case LDSBA:
        case LDSHA:
        case LDUBA:
...
void CDtm::CompleteBufferT(CRobItem* _robit)
{
  if( !EmptyBufferT() ) 
  {
    ...
    BufferT.AddMemInst( _robit->RType,
			_robit->WType,
			_robit->MAddr,
			(CIDecctl*) & _robit->Rd_hi,
			(CIDecctl*) & _robit->Rd_lo );
...
#if(DEBUG <= 14)
    printf( "\nDTM::STAGE_DS3::TR-LDST(seq=%ld, addr=%lx)",
	     _robit->GetSeqNum(),
	     BufferT.PC );
    BufferT.Display();
    getchar();
...
bool_t CDtm::ContextOverflow(CRobItem* _robit)
{
  ...
  {
    // campo do contexto de antrada usado para armazenar endereco
...
void CDtm::ReservRegs(CRsItem* _rsit, CRobItem* _robit, CMemoTableEntryT* _it)
{
  ...
  if( (_it->Rd_hi).IsReqAsSource() ) {
    if( IReg->IsBusy(_rsit->GetSourceWin(), (_it->Rd_hi).GetRegAddr()) )
      (_it->Rd_hi).SetDep( (_it->Rd_hi).GetRegAddr(),
			   IReg->GetSeqNum(_rsit->GetSourceWin(), (_it->Rd_hi).GetRegAddr()) );
    else
      (_it->Rd_hi).SetValue( IReg->GetValue(_rsit->GetSourceWin(), (_it->Rd_hi).GetRegAddr()) );
  }

  if( (_it->Rd_lo).IsReqAsSource() ) {
    if( IReg->IsBusy(_rsit->GetSourceWin(), (_it->Rd_lo).GetRegAddr()) )
      (_it->Rd_lo).SetDep( (_it->Rd_lo).GetRegAddr(),
			   IReg->GetSeqNum(_rsit->GetSourceWin(), (_it->Rd_lo).GetRegAddr()) );
    else
      (_it->Rd_lo).SetValue( IReg->GetValue(_rsit->GetSourceWin(), (_it->Rd_lo).GetRegAddr()) );
  }
  ...
  if( (_it->Rd_hi).IsReqAsTarget() )
    IReg->SetBusy(_rsit->GetTargetWin(), (_it->Rd_hi).GetRegAddr(), _rsit->GetSeqNum());

  if((_it->Rd_lo).IsReqAsTarget())
    IReg->SetBusy(_rsit->GetTargetWin(), (_it->Rd_lo).GetRegAddr(), _rsit->GetSeqNum());
...
bool_t CDtm::Stage_DS2(CDisqItem* _di)
{
  ...
  while(q != 0)
  {
    ...
    if(redundant == TRUE) {        // se existir algum trace redundante
      ...
      if( !q->IsLdStInst() ) {       // se trace NAO possuir instrucao de load/store
	...
	// (5) verifica se padrao de desvios coincide com fluxo no pipeline
	...
	return redundant;
      ...
      else {    // se trace possuir instrucao de load/store
	...
	// (6) marca registradores destino como ocupados
...
bool_t CDtm::Stage_DS3_TraceConstruction(CRobItem* _robit)
{
  ...
  if( IsValidDtmInst(_robit) ) 
  {  
    // se instrucao pertence ao dominio de instrucoes validas do DTM

    if( !_robit->IsDelaySlotInst() ) 
    {  
      // se instrucao NAO for de delay slot
...
//#if(DEBUG <= 11)
//	  fprintf( stderr,
//		   "\nDTM::STAGE_DS3::IR-LDST(seq=%ld, addr=%ld)",
//		   _robit->GetSeqNum(),
//		   _robit->GetInstAddr() );
//	  getchar();
//#endif
    ...
    {
      // se instrucao for de delay slot
  ...
  {
    // se instrucao NAO pertence ao conjunto de instrucoes validas do DTM
...
void CDtm::SetDepMap(CRobItem* _robit)
{
  ...
  else 
  {
    ...
    {
      if( (q->Rd_hi).IsReqAsTarget() )
	DepMap[ IREG_GETABSADDR(_robit->GetTargetWin(), (q->Rd_hi).GetRegAddr()) ] = TLD_TRACE_LOAD;

      if( (q->Rd_lo).IsReqAsTarget() )
	DepMap[ IREG_GETABSADDR(_robit->GetTargetWin(), (q->Rd_lo).GetRegAddr()) ] = TLD_TRACE_LOAD;
    }
