...
int CTrap::Stat()
{
  ...
  InvDtmTables(RdIReg(O1), sizeof(struct statbuf));
...
int CTrap::SigBlock()
{
  ...
  printf("trap::sigblock(maxk=%d,ret=%d\n", mask, ret);
...
int CTrap::Getrlimit()
{
  ...
  InvDtmTables(RdIReg(O1), sizeof(struct rlimit));
...
int CTrap::Mmap()
{
  ...
  printf("trap::mmap(addr=%ld,len=%d,prot=%d,flags=%d,fd=%d,off=%ld\n", 
    (uword_t)addr, 
    len,
    prot,
    flags,
...
int CTrap::Mount()
{
  ...
  printf("trap::mount(spetialfile=%ld,dir=%ld,rwflag=%d\n", (uword_t)specialfile, (uword_t)dir, rwflag);
...
int CTrap::Gettimeofday()
{
  ...
  InvDtmTables(RdIReg(O0), sizeof(struct timeval));
  InvDtmTables(RdIReg(O1), sizeof(struct timezone));
