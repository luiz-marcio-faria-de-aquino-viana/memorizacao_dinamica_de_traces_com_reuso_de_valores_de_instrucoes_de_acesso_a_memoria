...
void CMemoTableEntryT::Invalidate(uword_t _ptr, uword_t _len)
{
  int i;

  for(i = 0; i < LdStSz; i++) {

    if( ((LdStInst[i].LdSt & IOM_LOAD) == IOM_LOAD) && !LdStInst[i].LdStChain ) {
      ...
      switch( LdStInst[i].Type ) {
      case RD_BYTE:
      case RD_UBYTE:
	if( ((LdStInst[i].MAddr + sizeof(byte_t) - 1) >= _ptr) &&
	    (LdStInst[i].MAddr <= (_ptr + _len)) )
	  LdStInst[i].MValid = FALSE;
	break;
	  
      case RD_HWORD:
      case RD_UHWORD:
	if( ((LdStInst[i].MAddr + sizeof(hword_t) - 1) >= _ptr) &&
	    (LdStInst[i].MAddr <= (_ptr + _len)) )
	  LdStInst[i].MValid = FALSE;
	break;
	
      case RD_WORD:
	if( ((LdStInst[i].MAddr + sizeof(word_t) - 1) >= _ptr) &&
	    (LdStInst[i].MAddr <= (_ptr + _len)) )
	  LdStInst[i].MValid = FALSE;
	break;
	
      case RD_DWORD:
	if( ((LdStInst[i].MAddr + sizeof(dword_t) - 1) >= _ptr) &&
	    (LdStInst[i].MAddr <= (_ptr + _len)) )
	  LdStInst[i].MValid = FALSE;
	break;
...
void CMemoTableEntryT::UpdateMemValue(uword_t _maddr, ubyte_t * _ptr, uword_t _len)
{
  dword_t res;
  uword_t pos, n;
  ...
  for(i = 0; i < LdStSz; i++) {
	...
	n = sizeof(dword_t);
      ...
      res = mask = 0;
      if( ( LdStInst[i].MAddr <= (_maddr + _len - 1) ) && 
	  ( (LdStInst[i].MAddr + n - 1) >= _maddr ) ) {
	for(uword_t k = 0; k < n; k++) {
	  if( ( (LdStInst[i].MAddr + k) >= _maddr ) &&
	      ( (LdStInst[i].MAddr + k) <= (_maddr + _len - 1) ) ) {
	    pos = LdStInst[i].MAddr - _maddr + k;
	    res = (res << 8) | _ptr[pos];
	    mask = (mask << 8) | 0x0FFL;
#if(DEBUG <= 19)
	...
	if(LdStInst[i].Type != RD_DWORD) {
	  // hi
	  hi_mask = (uword_t) (mask & 0x0FFFFFFFFL);
	  LdStInst[i].MValhi &= ~hi_mask;
	  LdStInst[i].MValhi |= (uword_t) (res & 0x0FFFFFFFFL);
	  // lo
	  LdStInst[i].MVallo = 0;
	}
	else {
	  ...
	  LdStInst[i].MValhi &= ~hi_mask;
	  LdStInst[i].MValhi |= (uword_t) ((res >> 32) & 0x0FFFFFFFFL);
	  // lo
	  lo_mask = (uword_t) (mask & 0x0FFFFFFFFL);
	  LdStInst[i].MVallo &= ~lo_mask;
	  LdStInst[i].MVallo |= (uword_t) (res & 0x0FFFFFFFFL);
	}
...
void CMemoTableT::UpdateMemValue(uword_t _maddr, ubyte_t * _ptr, uword_t _len)
{
  ...
  p = LRUList.p_mru;
  while(p != NULL) {
    p->UpdateMemValue(_maddr, _ptr, _len);
    p = p->next;
  }  
}
