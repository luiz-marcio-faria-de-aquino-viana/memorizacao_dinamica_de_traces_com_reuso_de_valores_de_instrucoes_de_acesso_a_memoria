...
void CRobItem::InvalidateMem(uword_t _maddr, uword_t _nbytes)
{
  ...
  if( !IsTraceRedundant() ) {
    ...
    // se instrucao inicia um trace

    TraceData.Invalidate(_maddr, _nbytes);    
...
void CRobItem::UpdateMem(uword_t _maddr, ubyte_t * _val, uword_t _nbytes)
{
  ...
  if( !IsTraceRedundant() ) 
  {
	...
	n = sizeof(dword_t);
      ...
      /* antes de mexer com endianess nesta funcao lembre-se...
	 resolver endianess pode ser mais complexo do que parece */
	...
	if(_maddr == 0x0b5730) {
	  fprintf( stderr,
		   "\nROB::UPDATE_MEM_VALUE(ST - maddr=%lx, n=%ld)\n",
		   _maddr,
		   _nbytes );
	  
	  fprintf( stderr,
		   "\nROB::UPDATE_MEM_VALUE(seq=%ld, maddr=%lx, rtype=%ld, n=%ld, res=%016Lx, mask=%016Lx)\n",
		   GetSeqNum(),
		   MAddr,
		   RType,
		   n,
	...
	if(RType != RD_DWORD) {
	  ...
	  MValhi &= ~hi_mask;
	  MValhi |= (uword_t) (res & 0x0FFFFFFFFL);
	  // lo
	  MVallo = 0;
	}
	else {
	  ...
	  MValhi &= ~hi_mask;
	  MValhi |= (uword_t) ((res >> 32) & 0x0FFFFFFFFL);
	  // lo
	  lo_mask = (uword_t) (mask & 0x0FFFFFFFFL);
	  MVallo &= ~lo_mask;
	  MVallo |= (uword_t) (res & 0x0FFFFFFFFL);
	}
    ...
    // traces sao atualizados diretamente durante o acesso a memoria (vide: mem e lsu)
    // TraceData.UpdateMemValue(_maddr, _val, _nbytes);
  }
}
...
#if(DEBUG <= 21)
      printf(" (mvalhi=%lx, mvallo=%lx)", p->MValhi, p->MVallo);
#endif
