...
int CTrap::Open()
{
  ...
  printf("trap::open(path=%ld,flags=%d,mode=%d,ret=%d)\n", (uword_t)path, flags, mode, ret);
...
int CTrap::Lseek()
{
  ...
  printf("trap::lseek(fd=%d,offset=%ld,whence=%d,ret=%ld)\n", fd, (uword_t)offset, whence, (uword_t)ret);
...
int CTrap::Read()
{
  ...
  printf("trap::read(fd=%d,buf=%ld,nbyte=%d,ret=%d)\n", fd, (uword_t)buf, nbyte, ret);
...
int CTrap::Write()
{
  ...
  printf("trap::write(fd=%d,buf=%ld,nbyte=%d,ret=%d)\n", fd, (uword_t)buf, nbyte, ret);
...
int CTrap::Writev()
{
  ...
  printf("trap::writev(fd=%d,iov=%ld,iovcnt=%d,ret=%d)\n", fd, (uword_t)iov, iovcnt, ret);
...
int CTrap::Creat()
{
  ...
  printf("trap::write(path=%ld,mode=%d,ret=%d)\n", (uword_t)path, mode, ret);
...
int CTrap::Link()
{
  ...
  printf("path1=%ld,path2=%ld,ret=%d\n", (uword_t)path1, (uword_t)path2, ret);
...
int CTrap::Chown()
{
  ...
  printf("path=%ld,owner=%d,group=%d,ret=%d\n", (uword_t)path, owner, group, ret);
...
int CTrap::Fchown()
{
  ...
  printf("fd=%d,owner=%d,group=%d,ret=%d\n", fd, owner, group, ret);
...
int CTrap::Rename()
{
  ...
  printf("from=%ld,to=%ld,ret=%d\n", (uword_t)from, (uword_t)to, ret);
...
int CTrap::Truncate()
{
  ...
  printf("path=%ld,length=%d,ret=%d\n", (uword_t)path, length, ret);
...
int CTrap::SigVec()
{
  ...
  printf("sig=%d,vec=%ld,oldvec=%ld,ret=%d\n", sig, (uword_t)vec, (uword_t)oldvec, ret);
...
int CTrap::Ioctl()
{
  ...
  uword_t ptr = (uword_t)arg;
  ret = ss_ioctl(fd, cmd, (caddr_t)ptr);

  printf("\nioctl(fd=%d, cmd=%x, arg=%d, ret=%d)", fd, cmd, arg, ret);
...
int CTrap::Getrlimit()
{
  ...
  printf("trap::getrlimit(resource=%d,rlp=%ld,ret=%d)\n", resource, (uword_t)rlp, ret);
...
int CTrap::Setrlimit()
{
  ...
  printf("trap::setrlimit(resource=%d,rlp=%ld,ret=%d)\n", resource, (uword_t)rlp, ret);
...
int CTrap::GetDTableSize()
{
  ...
  printf("trap::getdtablesize(size=%d)\n", size);
...
int CTrap::Gettimeofday()
{
  ...
  printf("trap::gettimeofday(ret=%d)\n", ret);
