...
void CWb::Execute(udword_t* _nsteps)
{
  ...
  for(i = 0; i < WB_WIDTH; i++) 
  {
    ...
    robit = Rob->GetReadyItem();
    if(robit == NULL) break;

    if( robit->IsBadInst() ) {
      //// TESTE INSTRUCAO NAO IMPLEMENTADA - ERRO DE COMPILACAO COM 64-BITS (COMPILE E LINK COM -m32)
      sprintf(errtile, "- WB::EXECUTE(@%08lX, %08lX)", robit->Addr, robit->Inst);
      errmsg(__WB_H, ERR_NOTIMPLEMENTED, errtile);
    }
	...
	if(((CDtm*) Dtm)->BufferT.Tracesz > 0)
      ...
      Stat->AddTraceSz((robit->TraceData).Tracesz, TSZ_TRACE_REUSE, & ((CDtm*) Dtm)->BufferT.TraceStats);
