...
public:
  ...
  void AddInstrCount(uword_t _execunit, uword_t _instop, long _numinstr = 1L, int _tpfaixa = IC_NORMAL);  

  // AddInstrCount(): contabilizacao dos tipos de instrucoes por faixa
  // _stats - estatistica a ser contabilizada emforma de estrutura 'statistics_t'
  // _tpfaixa - tipo de faixa (0-NORMAL, 1-INST_REUSE, 2-TRACE, 3-TRACE_REUSE)
  void AddInstrCount(statistics_t * _stats, int _tpfaixa = IC_NORMAL);  
  ...
  // _stats - estatistica a ser contabilizada emforma de estrutura 'statistics_t'
  void AddTraceSz(long _tracesz, int _tpfaixa, statistics_t * _stats = NULL);
