...
CMainApp::CMainApp()
{
  ...
  ShowDtm = OPT_SHOW_DTM_DISABLE;
  cstdin = stdin;
}

int CMainApp::GetUserInput(const char* s, int n)
{
  int c;
  printf("\n%s: ", s);
  do {
...
uword_t CMainApp::SetShowDtm(int opt)
{
  ShowDtm = (opt == OPT_SHOW_DTM_DISABLE) ? 0 : ShowDtm | (1 << (opt - 1));
#if(DEBUG <= 22)
  fprintf(stderr, "\nMAIN::SETSHOWDTM(ShowDtm=%lu, opt=%d)\n", ShowDtm, opt);
#endif
  return ShowDtm;
}

void CMainApp::SetStdin(const char* filename)
{
...
void CMainApp::Run()
{
  ...
  do {
    opt1 = GetUserInput("A)Quit B)Load C)Args D)Reset E)Run F)Runn G)Step H)Show I)Watch J)Re-display K)Breakpt L)Trace M)Reset Stat N)Dtm Ctrl", OPT_DTMCTRL);
    switch(opt1) {
    case OPT_LOAD:
      printf("\nFile to load: ");
      fscanf(cstdin, "%s", s);
    ...
    case OPT_REDISPLAY:
      ...
      break;
    case OPT_SHOW:
      opt2 = GetUserInput(" >> A)Disable B)Program C)Args D)Mem E)BTB F)Fetch G)IQueue H)Disq I)DBDisq\n    J)IReg K)SReg L)Rs M)Iu N)Lsu O)Bpu P)Maq Q)Rob R)Comp.Trace S)Stat T)Dtm", OPT_SHOW_DTM);
      SetShowOptions(opt2);
      switch(opt2) {
      case OPT_SHOW_MEM:
	printf("\nMemory address: ");
	...
	ShowFollowPC = GetUserInput(" >> A)Base Address B)Follow PC", OPT_SHOW_PROGRAM_FOLLOWPC);
	...
	break;
      case OPT_SHOW_RS:
	SetShowRs( GetUserInput(" >> A)DISABLE B)IU C)BPU D)LSU", OPT_SHOW_RS_LSU) );
	break;
      case OPT_SHOW_IREG:
	ShowIReg = GetUserInput(" >> A)ALL B)BUSY ONLY", OPT_SHOW_IREG_BUSY);
	break;
      case OPT_SHOW_SREG:
	ShowSReg = GetUserInput(" >> A)ALL B)BUSY ONLY", OPT_SHOW_SREG_BUSY);
	break;
      case OPT_SHOW_DTM:
	opt3 = GetUserInput(" >> A)DISABLE B)MEMO_TABLE_G C)MEMO_TABLE_T D)MEMO_TABLE_L", OPT_SHOW_DTM_L);
	SetShowDtm(opt3);
      }
      break;
    case OPT_WATCH:
      ...
      break;
    case OPT_BREAKPT:
      switch(GetUserInput(" >> A)Disable B)Program C)Memory D)DTM", OPT_BREAKPT_DTM)) {
      case OPT_BREAKPT_DISABLE:
	Cpu.SetBreakpt();
	Cpu.SetDtmBreakCounter();
	Cpu.SetMemBreakpt();
	break;
      case OPT_BREAKPT_PROGRAM:
	printf("\nMemory address: ");
	fscanf(cstdin, "%i", & n);      
	Cpu.SetBreakpt(n);
	break;
      case OPT_BREAKPT_MEMORY:
	printf("\nMemory address: ");
	fscanf(cstdin, "%i", & n);      
	Cpu.SetMemBreakpt(n);
	break;
      case OPT_BREAKPT_DTM:
	printf("\nMemory address: ");
	fscanf(cstdin, "%i", & n);      
	Cpu.SetDtmBreakCounter(n);
      }
      break;

    case OPT_TRACE:
      switch(GetUserInput(" >> A)Disable B)Instructions C)Basic Blocks D) SIMSparc", OPT_TRACE_SIMSPARC)) {
    ...
    case OPT_RSTSTAT:
      ...
      break;
    case OPT_DTMCTRL:
      switch( GetUserInput(" >> A)Disable  B)DTM C)DTMm", OPT_DTMCTRL_DTMM) ) {
      case OPT_DTMCTRL_DISABLE:
	Cpu.Dtm.SetEnabled(DTM_DISABLED);
	printf("\nDTM is disabled.");
	break;
      case OPT_DTMCTRL_DTM:
	Cpu.Dtm.SetEnabled(DTM_ENABLED);
	printf("\nDTM is enabled.");
	break;
      case OPT_DTMCTRL_DTMM:
	Cpu.Dtm.SetEnabled(DTM_WITH_MEM_ENABLED);
	printf("\nDTMm is enabled.");
      }
    }
  } while(opt1 != OPT_QUIT);
...
void CMainApp::Show()
{
  ...
  if( BITTEST(ShowDtm, 1 << (OPT_SHOW_DTM_L - 1)) )
    (Cpu.Dtm).ShowMemoTableL();
}
